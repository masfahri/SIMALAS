            ...
            <div class="row">
                        ...
                        <div class="box-body">
                            <div class="mailbox-controls">
                                ...
                                <div class="pull-right">
                                    <div class="btn-group">
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'senin' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'senin')) }}"
                                                style="color: white">Senin</a></button>
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'selasa' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'selasa')) }}"
                                                style="color: white">Selasa</a></button>
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'rabu' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'rabu')) }}"
                                                style="color: white">Rabu</a></button>
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'kamis' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'kamis')) }}"
                                                style="color: white">Kamis</a></button>
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'jumat' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'jumat')) }}"
                                                style="color: white">Jumat</a></button>
                                        <button type="button"
                                            class="btn btn-primary btn-sm {{ Request::segment(5) == 'sabtu' ? 'active' : '' }}"><a
                                                href="{{ route('admin.master.jadwal.hari', array(Request::segment(4), 'sabtu')) }}"
                                                style="color: white">Sabtu</a></button>
                                    </div>
                                </div>
                            </div>
                            <div class="mailbox-messages inbox-bx">
                                <div class="table-responsive">
                                    ...
                                    <br>
                                    <center>
                                        <h1>Mohon Pilih Hari</h1>
                                        <center>
                                            @else
                                            @if ($data == null)
                                            <br>
                                            <center>
                                                <h1>Belum Ada Jadwal Dipilih</h1>
                                                <center>
                                                    @else
                ...
                <div class="col-xl-4 col-lg-4 col-12">
                    <div class="box">
                        ...
                        <h5 class="nav-link"><i class="fa fa-circle-o text-danger"></i>{{ $kelas->kd_kelas }} - {{
                            $kelas->kd_sub_kelas }} ({{ $kelas->Guru->GuruToUser->name }})</h5>
                        <!-- /.box-body -->
                    </div>
                    <!-- /.box -->
                    <div class="box">
                        ...
                        <div class="box-body no-padding mailbox-nav">
                            <div class="container">
                                <form action="{{ route('admin.master.jadwal.store', Request::segment(4)) }}"
                                    method="post">
                                    <br>
                                    @csrf
                                    <div class="form-group">
                                        ...
                                        <select name="hari"
                                            class="form-control {{ Request::segment(5) != null ? '' : 'select2' }}"
                                            readonly>
                                            <option value="">Pilih Hari</option>
                                            <option {{ Request::segment(5)=='senin' ? 'selected' : '' }} value="Senin">
                                                Senin</option>
                                            <option {{ Request::segment(5)=='selasa' ? 'selected' : '' }}
                                                value="Selasa">Selasa</option>
                                            <option {{ Request::segment(5)=='rabu' ? 'selected' : '' }} value="Rabu">
                                                Rabu</option>
                                            <option {{ Request::segment(5)=='kamis' ? 'selected' : '' }} value="Kamis">
                                                Kamis</option>
                                            <option {{ Request::segment(5)=='jumat' ? 'selected' : '' }} value="Jumat">
                                                Jumat</option>
                                            <option {{ Request::segment(5)=='sabtu' ? 'selected' : '' }} value="Sabtu">
                                                Sabtu</option>
                                        </select>
                                    </div>
                                    {!! Form::hidden('kd_kelas_sub_jur', Request::segment(4)) !!}
                                    <label for="pilihmapel">Pilih Mata Pelajaran</label><br>
                                    @for ($i = 0; $i < count($mapels); $i++) <div class="form-group">
                                        <input type="checkbox" id="{{ $mapels[$i]->kd_mapel }}" name="kd_mapels[]"
                                            value="{{ $mapels[$i]->kd_mapel }}">
                                        <label for="{{ $mapels[$i]->kd_mapel }}"> {{ $mapels[$i]->nama_mapel
                                            }}</label><br>
